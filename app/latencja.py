from collections import deque # standardowa biblioteka do obsługi kolejek o stałej długości
import threading # standardowa biblioteka do obsługi wątków

# Klasa do mierzenia i obliczania średniej oraz jitteru dla opóźnień API i czasu oczekiwania w kolejce.
class LatencyJitterMeter:
    def __init__(self, max_samples: int = 300):
        self._api_latency_ms = deque(maxlen=max_samples)
        self._admit_latency_ms = deque(maxlen=max_samples)
        self._prio_latency_ms = deque(maxlen=max_samples)
        self._queue_wait_s = deque(maxlen=max_samples)
        self._race_condition_count = 0
        self._lock = threading.Lock() # blokada do synchronizacji dostępu do kolejek, aby zapewnić bezpieczeństwo wątkowe podczas dodawania nowych wartości i tworzenia migawki danych.

    # Metoda pomocnicza do obliczania średniej z listy wartości.
    @staticmethod
    def _avg(values):
        return (sum(values) / len(values)) if values else 0.0

    # Metoda pomocnicza do obliczania jitteru, czyli średniej różnicy między kolejnymi wartościami.
    @staticmethod
    def _jitter(values):
        if len(values) < 2:
            return 0.0
        diffs = [abs(values[i] - values[i - 1]) for i in range(1, len(values))]
        return sum(diffs) / len(diffs)

    # Metoda do rejestrowania opóźnienia API w milisekundach.
    def record_api_latency_ms(self, value: float):
        #with self._lock:
            self._api_latency_ms.append(float(value))

    def record_admit_api_latency_ms(self, value: float):
        v = float(value)
        #with self._lock:
        self._admit_latency_ms.append(v)
        self._api_latency_ms.append(v)

    def record_prio_api_latency_ms(self, value: float):
        v = float(value)
        #with self._lock:
        self._prio_latency_ms.append(v)
        self._api_latency_ms.append(v)

    # Metoda do rejestrowania czasu oczekiwania w kolejce w sekundach.
    def record_queue_wait_s(self, value: float):
        #with self._lock:
            self._queue_wait_s.append(float(value))

    # Metoda do rejestrowania wykrytych konfliktów (np. HTTP 409).
    def record_race_condition(self, occurrences: int = 1):
        with self._lock:
            self._race_condition_count += max(1, int(occurrences))

    # Metody zwracające pełną historię pomiarów jako listę (do generowania wykresów).
    def get_admit_history(self) -> list[float]:
        return list(self._admit_latency_ms)

    def get_prio_history(self) -> list[float]:
        return list(self._prio_latency_ms)

    # Metoda do tworzenia migawki aktualnych wartości opóźnień i czasu oczekiwania.
    def snapshot(self):
        #with self._lock:
            api = list(self._api_latency_ms)
            admit = list(self._admit_latency_ms)
            prio = list(self._prio_latency_ms)
            qwait = list(self._queue_wait_s)

            with self._lock:
                race_count = int(self._race_condition_count)

            return {
            "api_latency_last_ms": round(api[-1], 2) if api else 0.0,
            "api_latency_avg_ms": round(self._avg(api), 2),
            "api_latency_jitter_ms": round(self._jitter(api), 2),

            "admit_latency_last_ms": round(admit[-1], 2) if admit else 0.0,
            "admit_latency_avg_ms": round(self._avg(admit), 2),
            "admit_latency_jitter_ms": round(self._jitter(admit), 2),

            "prio_latency_last_ms": round(prio[-1], 2) if prio else 0.0,
            "prio_latency_avg_ms": round(self._avg(prio), 2),
            "prio_latency_jitter_ms": round(self._jitter(prio), 2),

            "queue_wait_last_s": round(qwait[-1], 2) if qwait else 0.0,
            "queue_wait_avg_s": round(self._avg(qwait), 2),
            "queue_wait_jitter_s": round(self._jitter(qwait), 2),
            "race_condition_count": race_count,
        }
